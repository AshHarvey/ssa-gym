...
marwil_config2 = MARWIL_config.copy()
marwil_config2['evaluation_num_workers'] = 1
marwil_config2['num_workers'] = 12
marwil_config2['env_config'] = env_config
marwil_config2['evaluation_interval'] = 1
marwil_config2['train_batch_size'] = 4000
marwil_config2['rollout_fragment_length'] = 32
marwil_config2['evaluation_config'] = {'input': 'sampler'}
marwil_config2['beta'] = 1 # 0
marwil_config2['input'] = 'sampler'
if env_config['rso_count'] == 40:
    '''marwil_config2['model']['fcnet_hiddens'] = [512, 512]'''
    marwil_config2['rollout_fragment_length'] = 124

marwil_config2['explore'] = explore

MARWIL_trainer = MARWILTrainer(config=marwil_config, env=SSA_Tasker_Env)
MARWIL_trainer2 = MARWILTrainer(config=marwil_config2, env=SSA_Tasker_Env)
...
with MARWIL_trainer2.get_policy()._sess.graph.as_default():
    with MARWIL_trainer2.get_policy()._sess.as_default():
        MARWIL_trainer2.get_policy().model.base_model.load_weights("/tmp/pgr/weights3.h5")
...
while result['timesteps_total'] < num_steps_train:
    ...
    if result['training_iteration'] % 50 == 0:
        checkpoint = MARWIL_trainer2.save()
        ...
        output_path = MARWIL_trainer2._logdir + '/_running_results.pkl'
    ...
    if best_athlete > episode_len_mean[-1]:
        ...
        best_athlete_checkpoint = MARWIL_trainer2.save()
